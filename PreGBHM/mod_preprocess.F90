...
!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        infile="ws1km.asc"
        call smallextent(infile,infile,5,1)
    ...
    subroutine priver_horton()
      ...
      integer::nr,nc,ia1,ia2,ib1,ib2,outlet,ny,nx
      real(8)::x0,y0
...
!!!  Or eliminate small watershed here???
!!!
...
!cccccc determine the level ccccccc
      ...
      do k =1,level
        do i =1,numlev(k)
          j=j+1
          nstr='ws'
          write  (10,'(i4,3x,a2,i4,3x,i4.4)')j,nstr,k*1000+i,k*1000+i
        enddo
      enddo
      close(10)
      j=0
          ...
          do p = 1,nsub
            if(pbasin(p)== k*1000+i) then
              if(nbasinup(p) == 0) then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i ,0,0,0,0,0,0,0,0!,nextrowcol(p,1),nextrowcol(p,2),pbasin(basinpnext(p))
              elseif (nbasinup(p) == 1) then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i,pbasinup(p,1),0,0,0,0,0,0,0!,nextrowcol(p,1),nextrowcol(p,2),&
                       !pbasin(basinpnext(p))
              elseif  (nbasinup(p) == 2) then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i,pbasinup(p,1),pbasinup(p,2),0,0,0,0,0,0!,nextrowcol(p,1),nextrowcol(p,2),&
                       !pbasin(basinpnext(p))
              elseif (nbasinup(p) == 3)  then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i,pbasinup(p,1),pbasinup(p,2),pbasinup(p,3),0,0,0,0,0!,nextrowcol(p,1),&
                       !nextrowcol(p,2),pbasin(basinpnext(p))
              elseif (nbasinup(p) == 4)  then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i,pbasinup(p,1),pbasinup(p,2),pbasinup(p,3),pbasinup(p,4),0,0,0,0!,&
                       !nextrowcol(p,1),nextrowcol(p,2),pbasin(basinpnext(p))
              elseif (nbasinup(p) == 5)  then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i,pbasinup(p,1:5),0,0,0!,nextrowcol(p,1),nextrowcol(p,2),&
                       !pbasin(basinpnext(p))
              elseif (nbasinup(p) == 6)  then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i,pbasinup(p,1:6),0,0!,nextrowcol(p,1),nextrowcol(p,2),pbasin(basinpnext(p))
              elseif (nbasinup(p) == 7)  then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i,pbasinup(p,1:7),0!,nextrowcol(p,1),nextrowcol(p,2),pbasin(basinpnext(p))
              elseif (nbasinup(p) == 8)  then
                write  (10,'(i4,2x,i4,2x,11i6)')j,k*1000+i,pbasinup(p,1:8)!,nextrowcol(p,1),nextrowcol(p,2),pbasin(basinpnext(p))
            endif
            endif
          enddo
        enddo ! 
...
!    determine  pbasin number according to 
    ...
    subroutine deminput()
      ...
      if(.not. file_e) then
        ...
        call read_asc_head("dir1km.asc",NY,NX,X0,Y0,cellsize,nodata)
        allocate(dir1km(NY,NX))
        allocate(flowlen(NY,NX))
        call read_asc_int("dir1km.asc",NY,NX,dir1km)
        call flowlength(dir1km,flowlen,NX,NY,cellsize,nodata)
